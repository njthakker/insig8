...
#import <iostream>
#ifdef DEBUG
#import <insig8_debug-Swift.h>
#else
#import <insig8-Swift.h>
#endif
extern "C" {
...
void install(jsi::Runtime &rt,
  ...
  auto getWifiPassword = HOSTFN("getWifiPassword", []) {
    ...
    auto jsi_password = insig8::NSStringToJsiValue(rt, psk);
    auto jsi_ssid = insig8::NSStringToJsiValue(rt, ssid);
  ...
  auto ls = HOSTFN("ls", []) {
    NSString *path = insig8::jsiValueToNSString(rt, arguments[0]);
    NSError *error;
    NSArray *contents = [FS lsWithPath:path error:&error];

    if (error) {
      throw jsi::JSError(rt, insig8::NSStringToJsiValue(rt, error.description));
    }

    auto res = jsi::Array(rt, contents.count);

    for (int i = 0; i < contents.count; i++) {
      res.setValueAtIndex(rt, i, insig8::NSStringToJsiValue(rt, contents[i]));
    }
  ...
  auto readFile = HOSTFN("readFile", []) {
    ...
    return insig8::NSStringToJsiValue(rt, contents);
  });

  auto userName = HOSTFN("userName", []) {
    NSString *userName = NSUserName();
    return insig8::NSStringToJsiValue(rt, userName);
  });
  ...
  auto ps = HOSTFN("ps", []) {
    ...
    return insig8::NSStringToJsiValue(rt, string);
  });

  auto killProcess = HOSTFN("killProcess", []) {
    NSString *pid = insig8::jsiValueToNSString(rt, arguments[0]);
  ...
  auto log = HOSTFN("log", []) {
    NSString *msg = insig8::jsiValueToNSString(rt, arguments[0]);
    NSLog(@"%@", msg);
    return {};
  });
  ...
  rt.global().setProperty(rt, "__Insig8Proxy", std::move(module));
}
