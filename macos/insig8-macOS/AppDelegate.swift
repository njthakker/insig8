...
class AppDelegate: RCTAppDelegate {
  ...
  override func applicationDidFinishLaunching(_ notification: Notification) {
    self.moduleName = "insig8"
    self.initialProps = [:]
    self.automaticallyLoadReactNativeWindow = false
    super.applicationDidFinishLaunching(notification)

    let rootView = self.rootViewFactory().view(withModuleName: "insig8")
    ...
    ClipboardHelper.addOnCopyListener {
      ...
      if data != nil {
        ...
        do {
          ...
          Insig8Emitter.sharedInstance.fileCopied(filename, tempFile, bundle)
        } catch {
      ...
      if txt != nil {
        Insig8Emitter.sharedInstance.textCopied(txt!, bundle)
      }
    }
  }
